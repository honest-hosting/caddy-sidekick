...
| `SIDEKICK_CACHE_ITEM_MAX_SIZE` | Max size for any cached item (e.g., `100MB`, `0` = disabled, `-1` = unlimited) | `100MB` |
| `SIDEKICK_CACHE_STREAM_TO_DISK_SIZE` | Size threshold for streaming to disk (e.g., `10MB`, `0` = disabled) | `10MB` |
...
example.com {
    ...
    sidekick {
        ...
        cache_memory_max_size 128MB         # Total memory cache limit
        cache_memory_max_count 32768        # Max items in memory
        cache_item_max_size 100MB           # Max cacheable response size
        cache_stream_to_disk_size 10MB      # Stream to disk if larger than this
                      ...
                      "handle": [
                        {
                          ...
                          "cache_item_max_size": 104857600,
                          "cache_stream_to_disk_size": 10485760,
...
example.com {
    sidekick {
        ...
        cache_stream_to_disk_size 5MB    # Stream earlier to save memory
...
| `cache_item_max_size` | Prevent caching huge responses | `50MB` |
| `cache_stream_to_disk_size` | Balance memory vs disk I/O | `5MB` |
...
sidekick {
    ...
    cache_stream_to_disk_size 2MB
    cache_item_max_size 20MB
}
```
...
```caddyfile
sidekick {
    cache_memory_max_size 1GB
    cache_memory_max_count -1  # Unlimited count
    cache_stream_to_disk_size 20MB
    cache_item_max_size 200MB
}
```
...
2. Lower `cache_stream_to_disk_size`
3. Decrease `cache_memory_max_count`

### Disk space issues?
1. Reduce `cache_ttl`
2. Lower `cache_item_max_size`
3. Implement regular cache purging
